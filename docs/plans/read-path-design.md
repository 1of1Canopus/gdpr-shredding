...
### Cipher review of addendum 4, revision

**APPROVED WITH TWO CORRECTIONS.** All thirteen are applied faithfully and the round-trip assertion is a good addition. Everything else stands as written; no further review round — Thor builds once these two lines change and I verify them in the code.

1. **The parse must be the static `Identifier.toIdentifier(String)`, not the `IdentifierHelper`'s — my change 1 named the wrong one and I own that.** `NormalizingIdentifierHelperImpl.toIdentifier(text)` runs `normalizeQuoting`, which *adds* quoting the mapping never had whenever `globallyQuoteIdentifiers || autoQuoteKeywords && isReservedWord(text) || autoQuoteInitialUnderscore && text.startsWith("_") || autoQuoteDollar && text.contains("$")` (hibernate-core 7.4.5). §4.2's round-trip then diverges and refuses a valid application at startup, blaming "round-trip divergence" instead of the real reason: a column mapped unquoted and named `user` under `hibernate.auto_quote_keyword=true`, or named `_legacy_id`, or containing `$`. Use `org.hibernate.boot.model.naming.Identifier.toIdentifier(String)`: it reads the quoting present in the text and adds none, so the round-trip holds by construction for every column and still fails if the module ever mis-parses.
2. **§4.6 is wrong for the pairing Hibernate documents for JPA.** `globally_quoted_identifiers=true` with `globally_quoted_identifiers_skip_column_definitions=true` leaves columns *unquoted* at boot — `applyGlobalQuoting` honours the skip flag — while `normalizeQuoting` at runtime ignores it. "Every expression arrives quoted" is therefore false, and with correction 1 it need not be true: the module reproduces whatever the expression is. Restate §4.6 as *both settings supported for columns, quoted or unquoted, reproduced verbatim*; the table half is unchanged.

**The two checks asked for.** *Does a `StatelessSession` opened on an existing `Connection` share the transaction and snapshot in 7.4?* **Yes, by explicit design, not by luck.** `sessionFactory.withStatelessOptions().connection(c).openStatelessSession()` (`org.hibernate.StatelessSessionBuilder.connection(Connection)`) yields a `LogicalConnectionProvidedImpl` whose handling mode is `IMMEDIATE_ACQUISITION_AND_HOLD` and whose `close()` returns the connection without closing it — one physical connection, so one PostgreSQL transaction and one snapshot. `StatelessSessionImpl` sets `connectionProvided = options.getConnection() != null` and `isTransactionInProgress()` returns `connectionProvided || super…`, so `afterOperation()` never calls `afterTransaction()`: Hibernate treats a provided connection as caller-owned. It also cannot flush by construction — `flush()` is empty and `autoFlushIfRequired`/`autoPreFlushIfRequired` return `false`. Two hazards to write into §4.5: **(a)** never call `beginTransaction()` or `getTransaction().commit()` on that session — it would run `commit()` on the erasure's own connection and commit a half-done erasure (index cleared, key destroyed, no record appended, no residual verified), and `isTransactionInProgress()` already returning true means nothing stops the one-liner; **(b)** `LogicalConnectionProvidedImpl.afterCompletion()` calls `resetConnection(initiallyAutoCommit)`, a no-op only because `JdbcSupport.inTransaction` set `autoCommit=false` first — state that dependency, and close the session before the record is appended.

**Two probes added to the eighteen**, the rest kept verbatim: `probe_the_independence_check_never_commits_the_erasures_transaction` (the erasure's `Connection` is not committed by the stateless session; adding a `beginTransaction`/`commit` to the check turns it red) and `probe_an_auto_quoted_column_boots_and_round_trips` (an unquoted `user` column under `hibernate.auto_quote_keyword=true`, and `globally_quoted_identifiers=true` paired with `skip_column_definitions=true`: both boot and address the right column — red against the helper-based parse).
